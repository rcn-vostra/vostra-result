...
the **typed error identity survive the wire** — a test asserts `ShouldHaveError("Order.NotFound")` or
...
`Vostra.Results.AspNetCore` wire contract.
...
- New project `src/Vostra.Results.Testing/Vostra.Results.Testing.csproj`, TFMs **`net8.0;net9.0`**.
...
## 3. The wire contract we consume (from `Vostra.Results.AspNetCore`)
...
```
TestHttpClient ──uses──▶ IResultWireFormat ──uses──▶ WireErrorFactory
...
|------|----------------|-------------|
| `TestHttpClient` | issue HTTP verbs, own **request context** (verb, URL, body), map response→`Result` | `HttpClient`, `IResultWireFormat` |
| `IResultWireFormat` (+ `VostraJsonWireFormat`) | serialize requests; deserialize success payloads (strict on `T`); reconstruct raw errors | `System.Text.Json`, the envelopes, `WireErrorFactory` |
| `WireErrorFactory` | map `errorType`/`code`/`detail` (+ field map / error array) → typed `ErrorBase`(s) | Core error kinds |
...
The rich failure message (verb, URL, request body, server error — **S6**) needs data the *wire format never
sees*. Resolution:
1. `IResultWireFormat.ReadErrors` returns the **raw** reconstructed `ErrorBase` list (server identity only —
...
## 5. `IResultWireFormat` seam (FR-11.5) + reconstruction

```csharp
public interface IResultWireFormat
{
...
`VostraJsonWireFormat` uses `System.Text.Json`.
...
**`ReadErrors` / `WireErrorFactory`** — branch precisely:
...
2. **If `errorType == "Validation"`** → read `errors` as an **object map**; emit one `ValidationError` per
   field message with `code` = top-level `code` (per-field codes are not on the wire — documented limitation)
   and `metadata["field"]` = the map key.
...
```csharp
var api = new TestHttpClient(httpClient, baseUrl: "api/v2/details/admin");      // + optional IResultWireFormat
...
public sealed record PagedList<T>(IReadOnlyList<T> Items, Pagination Pagination);  // Pagination mirrors the wire
```
...
- URL joining: `baseUrl` + `url` with documented single-slash normalization (improves on the reference's naive
  concat).
- `PagedList<T>` re-declares a minimal `Pagination` (Testing has no AspNetCore ref); shape matches the wire.
...
- On failure, `VostraAssertionException.Message` is the S6 rich blob (verb, URL, request body, server error);
  the exception also carries the errors for debuggers.
...
- **Round-trip (primary, usage-first):** an in-process minimal-API host via `WebApplicationFactory` whose
  endpoints route through the **real** `ToHttpResponse`; `TestHttpClient` consumes that actual wire. Proves
  acceptance criterion #5 (`ShouldHaveError(...)` with no substring) and exercises every verb, success kind,
  list/pagination, and error shape end to end.
- **Unit (wire format):** `VostraJsonWireFormat`/`WireErrorFactory` against **canned JSON** for each shape —
...
| FR-11.2 | S6 rich message composed at throw-time; S7 strict-deser hint in wire format |
| FR-11.3 | `WireErrorFactory` rebuilds typed `ErrorBase` from `errorType`/`code` |
| FR-11.4 | zero-dep chainable `Assert`/`ShouldBeSuccess`/`ShouldHaveError` |
| FR-11.5 | `IResultWireFormat` seam (serializer + envelope), default STJ |
| FR-12   | consumes STJ wire, enum-as-string, case-tolerant |
...
2. **Newtonsoft `IResultWireFormat`** implementation (seam already present).
