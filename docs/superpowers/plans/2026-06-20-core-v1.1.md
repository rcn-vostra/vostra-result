# Vostra.Result Core v1.1 Implementation Plan
...
- Namespace `Vostra.Result` (lib), `Vostra.Result.Tests` (tests). Library BCL-only.
...
- Run tests from the repo root with the solution: `dotnet test Vostra.Result.sln`. If a run reports "Unable to find a project to restore", run `git restore Vostra.Result.sln` (the IDE intermittently strips its project entries) and retry.
...
**Files:**
- Rename: `src/Vostra.Result/Error.cs` → `src/Vostra.Result/ErrorBase.cs`
- Modify (content rename): `src/Vostra.Result/Errors.cs`, `ErrorType.cs`, `ResultSentinels.cs`, `ResultOfT.cs`, `ResultOfT.Inspection.cs`, `ResultOfT.Combinators.cs`, `ResultOfT.AsyncProjections.cs`, `ResultOfT.Linq.cs`, `Result.cs`, `ResultAggregate.cs`, `ResultAsyncExtensions.cs`, `ResultLinqAsyncExtensions.cs`
- Modify (tests): `tests/Vostra.Result.Tests/ErrorTests.cs`, `ResultOfTTests.cs`
...
Run: `git mv src/Vostra.Result/Error.cs src/Vostra.Result/ErrorBase.cs`

- [ ] **Step 4: Drop `Failure` from the `ErrorType` enum**

In `src/Vostra.Result/ErrorType.cs`, delete the `Failure` member and its XML doc line. The enum becomes:
```csharp
...
In `src/Vostra.Result/Errors.cs`, replace the `Error` class summary (formerly `UnexpectedError`) with:
```csharp
/// <summary>
...
**Files:**
- Create: `src/Vostra.Result/SuccessKind.cs`
- Modify: `src/Vostra.Result/ResultOfT.cs`, `src/Vostra.Result/Result.cs`
- Test: `tests/Vostra.Result.Tests/SuccessKindTests.cs`
...
- [ ] **Step 1: Write the failing tests `tests/Vostra.Result.Tests/SuccessKindTests.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~SuccessKindTests"`
Expected: FAIL — `SuccessKind`, `Result.Created` do not exist.

- [ ] **Step 3: Create `src/Vostra.Result/SuccessKind.cs`**
...
- [ ] **Step 4: Add `SuccessKind` to `Result<T>` in `src/Vostra.Result/ResultOfT.cs`**
...
- [ ] **Step 5: Add `SuccessKind` + `Created` factories to `Result` in `src/Vostra.Result/Result.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~SuccessKindTests"` → PASS (4).
Run: `dotnet test Vostra.Result.sln` → all pass, 0 warnings.
...
**Files:**
- Create: `src/Vostra.Result/Result.Inspection.cs`, `src/Vostra.Result/Result.Combinators.cs`
- Modify: `src/Vostra.Result/Result.cs` (add `partial`)
- Test: `tests/Vostra.Result.Tests/ResultCombinatorTests.cs`
...
- [ ] **Step 1: Write the failing tests `tests/Vostra.Result.Tests/ResultCombinatorTests.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ResultCombinatorTests"`
Expected: FAIL — these members do not exist.

- [ ] **Step 3: Make `Result` partial and expose `ErrorArray`**

In `src/Vostra.Result/Result.cs`, change `public readonly struct Result : IEquatable<Result>` to `public readonly partial struct Result : IEquatable<Result>`, and add (near `Errors`):
```csharp
    internal ErrorBase[] ErrorArray => _errors ?? ResultSentinels.UninitializedArray;
```

- [ ] **Step 4: Create `src/Vostra.Result/Result.Inspection.cs`**
...
- [ ] **Step 5: Create `src/Vostra.Result/Result.Combinators.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ResultCombinatorTests"` → PASS (7).
Run: `dotnet test Vostra.Result.sln` → all pass, 0 warnings.
...
**Files:**
- Create: `src/Vostra.Result/Result.AsyncExtensions.cs`
- Test: `tests/Vostra.Result.Tests/ResultAsyncTests.cs`
...
- [ ] **Step 1: Write the failing tests `tests/Vostra.Result.Tests/ResultAsyncTests.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ResultAsyncTests"`
Expected: FAIL — async overloads do not exist.

- [ ] **Step 3: Add instance async-projection methods to `Result.Combinators.cs`**

Append inside the `partial struct Result` in `src/Vostra.Result/Result.Combinators.cs`:
```csharp
...
- [ ] **Step 4: Create `src/Vostra.Result/Result.AsyncExtensions.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ResultAsyncTests"` → PASS (3).
Run: `dotnet test Vostra.Result.sln` → all pass, 0 warnings.
...
**Files:**
- Modify: `src/Vostra.Result/ErrorBase.cs` (add abstract `CloneWith` + the helpers), `src/Vostra.Result/Errors.cs` (override `CloneWith` in each subclass)
- Test: `tests/Vostra.Result.Tests/ErrorRewrapTests.cs`
...
- [ ] **Step 1: Write the failing tests `tests/Vostra.Result.Tests/ErrorRewrapTests.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ErrorRewrapTests"`
Expected: FAIL — `Prefix`/`WithCode`/`WithCausedBy` do not exist.

- [ ] **Step 3: Add `CloneWith` + helpers to `src/Vostra.Result/ErrorBase.cs`**
...
- [ ] **Step 4: Override `CloneWith` in each subclass in `src/Vostra.Result/Errors.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~ErrorRewrapTests"` → PASS (3).
Run: `dotnet test Vostra.Result.sln` → all pass, 0 warnings.
...
**Files:**
- Modify: `src/Vostra.Result/ResultAggregate.cs` (add `IEnumerable` `Combine` overloads + SelectAsync comment)
- Modify: `docs/superpowers/specs/2026-06-19-vostra-result-core-design.md` (equality text)
- Test: `tests/Vostra.Result.Tests/AggregationTests.cs` (add cases)
...
- [ ] **Step 1: Add failing tests to `tests/Vostra.Result.Tests/AggregationTests.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~AggregationTests"`
Expected: FAIL — `IEnumerable<...>.Combine()` does not exist.

- [ ] **Step 3: Add the collector overloads + the SelectAsync comment in `src/Vostra.Result/ResultAggregate.cs`**
...
Run: `dotnet test Vostra.Result.sln --filter "FullyQualifiedName~AggregationTests"` → PASS.
Run: `dotnet test Vostra.Result.sln` → all pass, 0 warnings.
