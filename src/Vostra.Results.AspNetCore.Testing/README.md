...
HTTP integration-testing toolkit for [Vostra.Results](https://www.nuget.org/packages/Vostra.Results). A
`TestHttpClient` collapses an HTTP round-trip into a `Result<T>`, rebuilding the **typed error** from the
`Vostra.Results.AspNetCore` RFC 7807 response so tests assert error *identity*, not substrings. It builds on
the transport-neutral chain-and-assert layer in
[Vostra.Results.Testing](https://www.nuget.org/packages/Vostra.Results.Testing).
...
- Assertions (from `Vostra.Results.Testing`) throw a `VostraAssertionException` with a rich diagnostic
