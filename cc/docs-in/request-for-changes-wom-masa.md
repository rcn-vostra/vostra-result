To the author of Vostra.Results,
  ...
  1. A transport-neutral version of the test-composition style. The single most attractive thing you've built, for us, is the chained-and-assert test shape — run a step, .Assert() it, BindAsync into the next step only if the previous one
  ...
  2. A way to keep going through several outcomes, not just stop at the first error. Our core loop processes a batch of actions and has to record every outcome — some are legitimate no-ops we skipped, some applied and advanced state, some
  ...
  3. Make several distinct failure kinds as first-class as several distinct success kinds. Your multi-success union (Result<T1,T2,T3>) is lovely for "one of several good shapes," but on the failure side everything collapses into one error
  ...
  4. Treat the exact human message as a first-class, guaranteed thing — not only the code. Your design leans on a stable error code surviving to the wire and into test assertions, which is great for HTTP APIs. But our contractors
